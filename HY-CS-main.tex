%r Follow comments to support use.
...
\usepackage[bibstyle=authoryear,backend=biber,natbib=true,maxnames=99,maxcitenames=2,uniquelist=minyear,giveninits=true,uniquename=mininit]{biblatex}
...
\usepackage{listings}
\usepackage{makecell}
\usepackage[pdftex]{hyperref} % For hyperlinks and pdf metadata
...
\title{Camera Architecture and Computational Photography}
...
\supervisors{Prof. Keijo Heljanko}

\keywords{Embedded Cameras, Camera API, Camera Sensors, Camera, Image Signal Processors}
...
\  CCS $\rightarrow$ Software and its engineering  $\rightarrow$ Software organization and properties\  \\
\  Software system structures  $\rightarrow$ Embedded software\\
}}
...
\hypersetup{
    ...
    pdfkeywords={camera} {isp}, % list of keywords for
...
\include{methology}
\include{Ch.20_History}
\include{Ch.30_Current_APIs}
\include{inteviews}
